...
**State as of 2026-08-14: spec phase complete, 10 implementation slices closed,
77 behaviour-named tests, CI green on every commit.**
...
**TEN / WF / SCAL / SYNC / PROC / OPS / MNT**: specified, not yet implemented.

## Next fronts (unordered candidates, groom before starting)

1. **§6 sync streams** — declared content dependencies over the feed; the
   converters from #11 supply SYNC-CONVERT-ON-APPLY
2. **SubscriptionTopic delivery** — completes EVT-FHIR-SUBSCRIPTIONS
3. **Tenant provisioning operator + planes** (TEN/WF beyond the schemas) —
   needs a k8s environment decision
4. **Maintenance export/import** (MNT) — backup=export over the feed fence
