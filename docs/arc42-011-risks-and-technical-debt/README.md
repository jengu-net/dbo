...
| [006 The specification is cut to the house style](006-the-specification-in-house-style/README.md) | Open. The stories' authored prose is cut, and the count turned out to measure two things: three quarters of a story is projected promise text. Next: that half, which is code. |
...
| [017 Quality coverage is not folded](017-quality-coverage-is-not-folded/README.md) | Open. The twelve goals are declared, one Quality each, and the promise report folds each one's coverage — performance reads 1/6, its sixth entry the gap where the figures are not taken. The quality tree's own table is still hand-kept. Next: project that table from the model. |
...
| [023 The suite runs out of heap](023-the-suite-runs-out-of-heap/README.md) | Open. Four tenants in three classes, across two documentation-only changes — one of them this item's own. What a tenant costs IS measured: 226 MB for a face's first, 11 MB for the next on it. The floor is attributed now — twelve classes keep 1755 MB and seventy more keep 477, against a 2 GB heap. Nothing is being left unclosed: the floor is the FHIR definition corpus, loaded once per face and resident by design. Next: whether one JVM holds three faces. |
