...
| [017 Quality coverage is not folded](017-quality-coverage-is-not-folded/README.md) | Open. The twelve goals are declared now, one Quality each, naming the promises that fulfil it. Nothing folds their coverage yet: the projection walks areas and a quality crosses them. Next: render the qualities, and replace the tree's table with that. |
