...
| [004 The guide runs three times in CI](004-the-guide-runs-three-times/README.md) | Open. The step is ported, the shell harness is gone from the tree run, and who moves the pin is written down. Decided on a count: the suite runs all ninety-nine published snippets, so the guide's shell run covers nothing it does not, while the quickstart's own run covers a compose file nothing else touches. Next: the deletion. |
