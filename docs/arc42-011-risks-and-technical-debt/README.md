...
| [024 Definitions out of the heap](024-definitions-out-of-the-heap/README.md) | Open. Nothing is built. A second face costs 444 MB on top of 225 for the first, and the aim is a serving process holding neither graph. The critical path is written out: nine moves, no partial win, and conversion turns out not to be on it. Next: the snapshot into the cut. |
