...
| [021 Asking the store a question](021-asking-the-store/README.md) | Open. The vocabulary exists with two bindings a caller cannot tell apart, now for work as well as records: the surface serves a run search, and both hand back an Ongoing — the seven fields of a run a rendering carries — rather than a Run whose other eleven only one binding could fill. The guide chapter reads over a ward the sample compiles. Next: the observer seam's chapter section. |
