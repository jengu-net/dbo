...
that was built or unnecessary, which is worth naming as a pattern rather than
fixing three times: rows written while planning stayed in the imperative long
after the thing was done, because nothing here derives its status from the
tree the way the promise catalogue does. A consumer reading this table ranked 4 above 2 on
