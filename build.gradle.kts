...
// ─── Re-recording ─────────────────────────────────────────────────────
//
// Eight artefacts are generated from something else and committed beside it,
// and each has a ratchet that fails the build when the committed copy
// disagrees. A ratchet speaks one commit late, on a file nobody was thinking
// about, so the rule asks for the re-record to be in the change that made it
// stale. Until now that meant remembering eight commands.
//
// The diagrams join only when their compiler is installed: it is an author's
// tool rather than a build dependency, and the SVGs are committed so the site
// never needs it. A skip is announced, because a silent one is how a
// committed drawing drifts away from its source.
fun liniInstalled(): Boolean =
    try {
        providers.exec {
            commandLine("bash", "-c", "command -v '" + liniBin + "' >/dev/null 2>&1")
...
val reRecord = tasks.register("reRecord") {
    ...
    dependsOn(
        generateSkills,
        moduleMap,
        ":core:harness:promiseProjection",
        ":core:harness:apiLedger",
        ":core:harness:reachLedger",
        ":core:harness:promiseCitations",
        ":core:harness:worldsLedger",
    )
    val drawings = liniInstalled()
    if (drawings) {
        dependsOn(siteDiagrams)
    }
    doLast {
        if (!drawings) {
            logger.lifecycle(
                "the diagrams were NOT re-recorded: '" + liniBin + "' is not on PATH. "
                    + "Install the pinned version, or pass -Plini=/path/to/lini, if a "
                    + ".lini source or a .desc changed in this commit.",
            )
        }
        logger.lifecycle("re-recorded; commit whatever moved, in this change")
    }
}

// The pre-commit hook, installed by the build rather than by remembering to.
//
// It is written at configuration time, so any invocation of this build puts it
// there — a hook a person has to install is a hook half the checkouts do not
// have, and the failure it prevents is invisible until CI.
//
// It is deliberately graded. A documentation commit pays a python script and
// nothing else; the Gradle recorders run only when a source they read is
// staged. A Gradle failure warns rather than blocks, because code that does
// not compile is the committer's business and the ratchets still hold the
// line in CI.
val preCommitHook = """
    #!/usr/bin/env bash
    # GENERATED by the build, from the hook block in build.gradle.kts.
    #
    # Re-records what this commit makes stale, and stages it. Set
    # DBO_SKIP_RERECORD=1 to skip.
    set -uo pipefail
    [ "${'$'}{DBO_SKIP_RERECORD:-}" = "1" ] && exit 0
    cd "${'$'}(git rev-parse --show-toplevel)" || exit 0
    staged=${'$'}(git diff --cached --name-only --diff-filter=ACMR)
    [ -z "${'$'}staged" ] && exit 0
    moved=""

    # A projection written by a python script costs nothing, so a changed
    # document always pays for it.
    if printf '%s\n' "${'$'}staged" | grep -q '^docs/.*\.md${'$'}'; then
        python3 scripts/generate-skills.py >/dev/null 2>&1 \
            && moved="${'$'}moved tools/dbo-conventions CLAUDE.md"
    fi

    if printf '%s\n' "${'$'}staged" | grep -qE '^docs/.*/diagrams/.*\.(lini|desc)${'$'}'; then
        if command -v lini >/dev/null 2>&1; then
            ./gradlew --no-daemon -q siteDiagrams >/dev/null \
                && moved="${'$'}moved site/assets/diagrams"
        else
            echo "pre-commit: a diagram source is staged and lini is not installed;" >&2
            echo "            the committed SVG may now disagree with it." >&2
        fi
    fi

    # Everything else is read from compiled classes or from the project model,
    # so it costs a build. Only a staged source of one of them asks for it.
    if printf '%s\n' "${'$'}staged" | grep -qE '\.(java|kts)${'$'}'; then
        if ./gradlew --no-daemon -q reRecord >/dev/null; then
            # Named, not the whole of config/: the baselines there are written
            # by tests, and a commit should not sweep up one nobody looked at.
            moved="${'$'}moved config/api-ledger.txt config/reach-ledger.txt"
            moved="${'$'}moved config/promise-citations.txt config/worlds-ledger.txt"
            moved="${'$'}moved config/module-map.txt tools/dbo-conventions CLAUDE.md"
            moved="${'$'}moved docs/arc42-006-runtime/req-catalogue.md"
            moved="${'$'}moved docs/arc42-003-context/user-stories"
        else
            echo "pre-commit: re-recording failed, so a projection may be stale." >&2
            echo "            The build says why; the ratchets will say so again." >&2
        fi
    fi

    [ -n "${'$'}moved" ] && git add -- ${'$'}moved >/dev/null 2>&1
    exit 0
""".trimIndent() + "\n"

val hooksDir = layout.projectDirectory.dir(".git/hooks").asFile
if (hooksDir.isDirectory) {
    val installed = File(hooksDir, "pre-commit")
    if (!installed.exists() || installed.readText() != preCommitHook) {
        // Written beside it and RENAMED over it, never written through.
        //
        // The hook runs this build, so this code can execute while bash is
        // part-way through reading the very file it would overwrite — and bash
        // reads a script incrementally, so it carries on at the old offset in
        // the new bytes and dies on a quote that is no longer there. A rename
        // swaps the directory entry and leaves the running interpreter its own
        // inode to finish.
        val staged = File(hooksDir, "pre-commit.next")
        staged.writeText(preCommitHook)
        staged.setExecutable(true)
        if (!staged.renameTo(installed)) {
            staged.delete()
            logger.warn("could not install the pre-commit hook")
        } else {
            logger.lifecycle("installed the pre-commit hook")
        }
    }
}
