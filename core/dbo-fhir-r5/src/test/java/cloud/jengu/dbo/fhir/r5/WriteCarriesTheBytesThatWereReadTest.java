...
class WriteCarriesTheBytesThatWereReadTest {
    ...
    private static R5Store storeInto(Capturing engine) {
        return new R5Store(engine, new R5Personality(List.of(
    ...
    void theEngineIsHandedTheArrayThatWasRead() {
        ...
        assertEquals(1, R5Version.READS.get() - before, "accepting one write read it twice");
        // the same array, so the face still holds its document for it
        long read = R5Version.READS.get();
        payloads().read("Patient", engine.captured.payload());
        assertEquals(read, R5Version.READS.get(),
